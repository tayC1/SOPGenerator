...
// Same workspace-wide scope as searchSops() above (no visibility filtering)
// - backs `/codex share` with no search term, for browsing/picking a SOP to
// share rather than searching for one.
async function listRecentSops({ limit = 8 } = {}) {
  const result = await db.query(
    `SELECT id, title, description, category, doc_type
     FROM sops
     ORDER BY created_at DESC
...
module.exports = { searchSops, listRecentSops };
