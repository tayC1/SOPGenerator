...
// Handles the three auth-bridge message types the web app sends
// (USER_SIGNED_IN / USER_SIGNED_OUT / EDIT_SOP). Reachable two ways:
//   - Chrome: chrome.runtime.onMessageExternal, gated by manifest.json's
//     externally_connectable allowlist.
//   - Safari (which has no externally_connectable support), and as a
//     fallback for Chrome too: content.js relays the same message shape
//     in in over the regular onMessage channel after receiving it via
//     window.postMessage from a trusted CODEX origin - see the
//     `source === 'codex-web'` branch below and the listener in content.js.
function handleAuthBridgeMessage(message, sender, sendResponse) {
  console.log(`[auth-bridge] "${message.type}" from ${sender.origin || sender.url || 'unknown'}`);
...
chrome.runtime.onMessageExternal.addListener((message, sender, sendResponse) =>
  handleAuthBridgeMessage(message, sender, sendResponse));
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  // Relayed from content.js's window.postMessage listener (see content.js) -
  // the postMessage fallback path for the same auth-bridge messages Chrome
  // normally delivers via onMessageExternal above. content.js has already
  // checked the page's origin against the trusted CODEX origin allowlist
  // before relaying, so this handler doesn't need to re-check it.
  if (request.source === 'codex-web') {
    return handleAuthBridgeMessage(request, sender, sendResponse);
  }
